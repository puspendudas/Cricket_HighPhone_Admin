...
export default function Dt20() {
  ...
  const title = 'DRAGON TIGER 20-20';

  const getWinnerName = (res: any) => {
    const r = String(res.result || res.win || '').toUpperCase();
    if (r === '1' || r === 'D' || r === 'DRAGON') return 'D';
    if (r === '2' || r === 'T' || r === 'TIGER') return 'T';
    if (r === '3' || r === 'TIE') return '-';
    return r || '?';
  };

  const getWinnerColor = (winner: string) => {
    if (winner === 'D') return '#FF4B4B'; // Red for Dragon
    if (winner === 'T') return '#FF9F43'; // Orange/Yellow for Tiger
    if (winner === '-') return '#16C75A'; // Green for Tie
    return '#757575';
  };
      ...
      <Grid container spacing={{ xs: 1.5, md: 3 }}>
        ...
        <Grid item xs={12} md={8}>
          ...
          <Card sx={{ borderRadius: '12px', overflow: 'hidden', mb: 3 }}>
            ...
            {(() => {
              const allowedNats = [
                'dragon', 'tiger', 'tie', 'pair', 
                'dragon even', 'dragon odd', 'dragon red', 'dragon black', 'dragon back',
                'tiger even', 'tiger odd', 'tiger red', 'tiger black', 'tiger back'
              ];
              const oddsRows = oddsData?.sub?.filter((item: any) => {
                const n = (item.nat || item.name || '').toLowerCase();
                return allowedNats.includes(n);
              }) || [];

              return oddsRows.map((player: any, idx: number) => {
                ...
                <Box key={idx} sx={{ display: 'flex', alignItems: 'center', p: 1.5, borderBottom: '1px solid #E5E7EB' }}>
                  <Box sx={{ flex: 1 }}>
                    <Typography sx={{ fontWeight: 'bold', color: '#1F2937' }}>
                      {player.nat || player.name || `PLAYER ${idx === 0 ? 'D' : 'T'}`}
                    </Typography>
                  </Box>
