...
export default function Teen() {
  ...
  let playerRows = oddsData?.sub?.filter((item: any) => item.subtype === "Player" || item.subtype === "teen") || [];
  if (playerRows.length === 0 && oddsData?.sub) {
    playerRows = oddsData.sub.filter((item: any) =>
      item.nat?.toLowerCase().includes("player") ||
      item.name?.toLowerCase().includes("player")
    );
  }
  if (playerRows.length === 0 && oddsData?.sub?.length >= 2) {
    playerRows = [
      { ...oddsData.sub[0], nat: oddsData.sub[0].nat || oddsData.sub[0].name || 'Player A' },
      { ...oddsData.sub[1], nat: oddsData.sub[1].nat || oddsData.sub[1].name || 'Player B' }
    ];
  }

  const cardsArr = oddsData?.card ? oddsData.card.split(',') : ['1', '1', '1', '1', '1', '1'];

  const title = 'TEENPATTI';
