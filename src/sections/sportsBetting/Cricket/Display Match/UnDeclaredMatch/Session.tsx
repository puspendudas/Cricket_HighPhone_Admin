...
const deleteMultipleBetsMutation = useMutation({
  ...
  const filteredData = useMemo(() => {
    ...
    return betHistoryData.data.filter((row: any) => {
      ...
      // ✅ PENDING aur DELETED dono status show honge
      if (row.status !== "PENDING" && row.status !== "DELETED") return false;

      // Apply team filter
      if (team && row.runner_name !== team) return false;
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      // Only select bets that are not deleted
      const selectableBets = filteredData.filter((bet: any) => bet.status !== "DELETED");
      setSelectedBets(selectableBets.map((bet: any) => bet.id));
    } else {
      setSelectedBets([]);
    }
  };
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              ...
              <Grid item xs={12} md={6}>
                <FormControl fullWidth>
                  ...
                  <Select value={team} label="Session" onChange={handleTeamChange}>
                    <MenuItem value="">
                      <em>All Sessions</em>
                    </MenuItem>
                    {teams.map((t: string) => (
                ...
                {loading ? (
                  ...
                  filteredData.map((row: any) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell>{row.odds_value}</TableCell>
