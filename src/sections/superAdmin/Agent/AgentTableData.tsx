...
import type { Member, Updatepaylod, MemberFormData } from 'src/Interface/agent.interface';
...
export function AgentTableData() {
  const { addAgent, fetchAagentList, updateAgent } = useSuperAdminApi();
  ...
  const fetchMembers = useCallback(async () => {
    try {
      const { admin } = await fetchAagentList();
      const formatted: Member[] = admin.map((a: any, i: number) => ({
        ...
        AgentWallet: `₹${a.parent_id.wallet}`,
  ...
  const handleEditSave = async (updatedData: Updatepaylod) => {
    ...
    try {
      await updateAgent(selectedMember._id, updatedData);
      await fetchMembers();
    ...
    <Paper sx={{ p: 2, boxShadow: 3 }}>
      ...
      <Box sx={{ overflowX: 'auto' }}>
        <Table>
          <TableHead>
            <TableRow>
              {["", "Action", "#", "User", "Name", "Password", "Agent", "Super Agent", "Current Bal",
                // "Client",
...
Password: ${selectedMember.password}
    `;
                ...
                <TableCell
                  ...
                  onClick={() => navigate(`/master/client/${member._id}`)}
                >
                  {member.user}
                </TableCell>
                ...
                {/* <TableCell>{member.AgentWallet}</TableCell> */}
