...
import type { Member, Updatepaylod, MemberFormData } from 'src/Interface/super_agent.interface';
...
export function SuperAgentTableData() {
  const { addSuperAgent, fetchSuperAgentList, updateSuperAgent } = useSuperAdminApi();
  ...
  const fetchMembers = useCallback(async () => {
    try {
      const { admin } = await fetchSuperAgentList();
      const formatted: Member[] = admin.map((a: any, i: number) => ({
        ...
        superAgentWallet: `₹${a.parent_id.wallet}`,
  ...
  const handleEditSave = async (updatedData: Updatepaylod) => {
    ...
    try {
      await updateSuperAgent(selectedMember._id, updatedData);
      await fetchMembers();
    ...
    <Paper sx={{ p: 2, boxShadow: 3 }}>
      ...
      <Box sx={{ overflowX: 'auto' }}>
        <Table>
          <TableHead>
            <TableRow>
              {["", "Action", "#", "User", "Name", "Password", "Super Agent", " Master", "Current Bal",
                //  "Agent",
...
Password: ${selectedMember.password}
    `;
                ...
                <TableCell
                  ...
                  onClick={() => navigate(`/master/agent/${member._id}`)}
                >
                  {member.user}
                </TableCell>
                ...
                {/* <TableCell>{member.superAgentWallet}</TableCell> */}
