...
import type { Member, Updatepaylod, MemberFormData } from 'src/Interface/super_master.interface';
...
export function SuperMasterTable() {
  const { addSuperMaster, fetchSuperMasterList, updateSuperMaster } = useSuperAdminApi();
  ...
  const fetchMembers = useCallback(async () => {
    try {
      const { admin } = await fetchSuperMasterList();
      const formatted: Member[] = admin.map((a: any, i: number) => ({
        ...
        superMasterWallet: `₹${a.parent_id.wallet}`,
  ...
  const handleEditSave = async (updatedData: Updatepaylod) => {
    ...
    try {
      await updateSuperMaster(selectedMember._id, updatedData);
      await fetchMembers();
    ...
    <Paper sx={{ p: 2, boxShadow: 3 }}>
      ...
      <Box sx={{ overflowX: 'auto' }}>
        <Table>
          <TableHead>
            <TableRow>
              {["", "Action", "#", "User", "Name", "Password", "Super Master", " Admin", "Current Bal",
                // " Master",
...
Password: ${selectedMember.password}
    `;
                ...
                <TableCell
                  ...
                  onClick={() => navigate(`/master/masters/${member._id}`)}
                >
                  {member.user}
                </TableCell>
                ...
                {/* <TableCell>{member.superMasterWallet}</TableCell> */}
