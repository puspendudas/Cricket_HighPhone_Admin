...
export function AdminTableData() {
  const { addadmin, fetchAdminList, updateAdmin } = useAdminApi();
  ...
  const fetchMembers = useCallback(async () => {
    try {
      const { admin } = await fetchAdminList();
      const formatted: Member[] = admin.map((a: any, i: number) => ({
        ...
        superAdminwallet: `₹${a.parent_id.wallet}`,
  ...
  const handleEditSave = async (updatedData: Updatepaylod) => {
    ...
    try {
      await updateAdmin(selectedMember._id, updatedData);
      await fetchMembers();
    ...
    <Paper sx={{ p: 2, boxShadow: 3 }}>
      ...
      <Box sx={{ overflowX: 'auto' }}>
        <Table>
          <TableHead>
            <TableRow>
              {["", "Action", "#", "User", "Name", "Password", "Admin", "Super Admin", "Current Bal",
          ...
          <TableBody>
            {currentRows.map((member) => (
              <TableRow key={member.id}>
                {/* Red icon agar user inactive ho */}
                <TableCell>
                  ...
                  >
                    ...
                    <MenuItem
                      onClick={() => {
                        setModalState((p) => ({
                          ...p,
                          memberForLimitUpdates: member.name,
                        }));
                        setModals((p) => ({ ...p, updateLimit: true }));
                        handleMenuClose();
                      }}
                    >
              ...
              Password: ${selectedMember.password}`;
                    ...
                    >
                      <Iconify icon="material-symbols:key-outline" sx={{ mr: 1 }} /> Send
                      Login Detail
                    </MenuItem>

                    <MenuItem
                      onClick={() => {
                        setModalState((p) => ({ ...p, memberForWD: member }));
                        setModals((p) => ({ ...p, wdLimit: true }));
                        handleMenuClose();
                      }}
                    >
                      <Iconify
                        icon="material-symbols:folder-limited-outline-sharp"
                        sx={{ mr: 1 }}
                      />{' '}
                      Limit W/D
                    </MenuItem>
                ...
                <TableCell
                  ...
                  onClick={() => navigate(`/master/super-master/${member._id}`)}
                >
                  {member.user}
                </TableCell>
